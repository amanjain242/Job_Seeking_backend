...
export const postApplication = catchAsyncError(async (req, res, next) => {
    ...
    const { name, email, coverLetter, phone, address, jobId ,jobTitle, companyName} = req.body;
    const applicantID = {
        user: req.user._id,
        role: "Job Seeker",
    };
    if (!jobId) {
        return next(new ErrorHandler("Job not found, error in the jobId!", 404));
    }
    const jobDetails = await Job.findById(jobId);
    if (!jobDetails) {
        return next(new ErrorHandler("Job not found in the job Schema!", 404));
    }
